...
const statusConfig: Record<string, { icon: any; color: string; label: string }> = {
  ...
  declined: { icon: X,            color: "text-destructive",       label: "Declined" },
...
<script>window.onload=()=>{window.print();window.onafterprint=()=>window.close()}</script>
  ...
  const resetForm = () => {
    ...
    setValidUntil(""); setNotes(""); setTaxRate(0); setDiscountAmount(0);
  ...
  const viewQuote = async (quote: Quote) => {
    setShowView(quote);
    const { data } = await supabase.from("invoice_items").select("*").eq("invoice_id", quote.id);
  ...
  const stats = {
    ...
    value: quotes.filter((q) => q.status === "accepted").reduce((s, q) => s + Number(q.total), 0),
  };
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground">Create and send professional proposals</p>
        </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        {[
          ...
          { label: "Won Value", value: fmt(stats.value) },
        ].map((s) => (
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input placeholder="Search quotes..." value={search} onChange={(e) => setSearch(e.target.value)} className="pl-9 rounded-lg" />
        </div>
        <Select value={filterStatus} onValueChange={setFilterStatus}>
          <SelectTrigger className="w-full sm:w-36 rounded-lg"><SelectValue /></SelectTrigger>
          <SelectContent>
      ...
      {loading ? (
        ...
        <div className="text-center py-14 border border-dashed border-border rounded-xl">
          ...
          <p className="font-display font-bold text-sm">No quotes yet</p>
          <p className="text-xs text-muted-foreground mt-1">Create a quote to send to a customer</p>
              ...
              >
                <div className="flex items-center gap-3 min-w-0">
                  ...
                  <div className="min-w-0">
                    <p className="font-display font-bold text-sm truncate">{q.customer_name}</p>
                    <p className="text-[10px] text-muted-foreground">{q.quote_number}</p>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="text-right hidden sm:block">
                    <p className="font-display font-bold text-sm">{fmt(Number(q.total))}</p>
                    <p className="text-[10px] text-muted-foreground">{new Date(q.issue_date).toLocaleDateString()}</p>
                  </div>
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      {q.status === "draft" && (
                        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); updateStatus(q.id, "sent"); }}>
                          <Send className="h-3.5 w-3.5 mr-2" /> Mark Sent
                        </DropdownMenuItem>
                      )}
                      {q.status === "sent" && (
                        <DropdownMenuItem onClick={(e) => { e.stopPropagation(); updateStatus(q.id, "accepted"); }}>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl">
          {showView && (
            <>
              <DialogHeader>
                <div className="flex items-center justify-between">
                  <DialogTitle className="font-display">Quote {showView.quote_number}</DialogTitle>
              ...
              <div className="mt-3 space-y-5">
                ...
                {showView.notes && (
                  <div>
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider font-medium mb-1">Notes / Terms</p>
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {showView.status === "sent" && (
                    <Button className="flex-1 rounded-lg" onClick={() => { updateStatus(showView.id, "accepted"); setShowView(null); }}>
                      <CheckCircle2 className="h-3.5 w-3.5 mr-1.5" /> Mark Accepted
                    </Button>
                  )}
                  {showView.status === "accepted" && (
                    <Button className="flex-1 rounded-lg" onClick={() => convertToInvoice(showView)}>
                      Convert to Invoice
                    </Button>
                  )}
